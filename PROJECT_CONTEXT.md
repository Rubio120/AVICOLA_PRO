...
Entregas 0-10 cerradas; Entregas 11 y 12 siguen sin certificación remota. El roadmap contiene 13 entradas porque enumera la Entrega 0 documental más las entregas de producto 1-12. Último run remoto: `35890371500`; falló al probar Restic en el constructor y omitió scans/Compose/restore/bundle. La corrección local del builder y el gate autenticado de release esperan un CI nuevo con el SHA final.
...
Entrega 12 incorpora bundle reproducible con archivos `docker save`, IDs de imagen, hashes, SBOM CycloneDX y reportes; GitHub attesta el `.tar` y el CLI verifica workflow/repositorio/commit/ref/subject y timestamp verificable. El gate local inspecciona el bundle con extracción segura, recalcula hashes, exige scans sin findings HIGH/CRITICAL, head Alembic y tag Git exacto. Localmente pasaron 246/246 pruebas backend (80,86 %), Ruff/formato/Mypy y pip-audit; frontend lint, TypeScript, build y npm audit pasan. Vitest local no pudo iniciar porque esbuild intentó acceder fuera del workspace. Solo propone `ready_for_user_deployment` con evidencia CI fresca y completa; no publica registry digests. Pilot sigue bloqueado por restore off-host, RPO/RTO y responsables aprobados. CI del SHA final sigue pendiente tras fallo de Restic en run `35890371500`. La revisión RBAC/aceptación externa sigue pendiente. El roundtrip sintético no es aceptación off-host y el esquema no tiene hash-chain criptográfico de auditoría.
...
Segundo CI remoto para `e056eae` (run 35871086303): PostgreSQL pasó con 209 pruebas aprobadas, 1 omitida y 80,86 % de cobertura; Windows, auditoría de dependencias y Trivy fuente también pasaron. Los tres SARIF de runtime finalizaron (backend 62, frontend 67, backup 162 resultados HIGH/CRITICAL), por lo que no se aceptan las imágenes. Compose falló en el comprobador con `KeyError: networks`; el siguiente run añade claves estructurales para diagnosticar. Se añadió `apt-get upgrade --yes` durante el build de runtime (13 pruebas unitarias locales de configuración pasan), aún sin validación de Docker. No se rebaja el gate de seguridad.
