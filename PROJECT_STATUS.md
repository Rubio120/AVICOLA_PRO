...
2026-09-23 - Entregas 0-10 cerradas; 11 en preparacion tecnica y 12 en curso, aun no certificadas. Suite backend CI PostgreSQL: 209 pasaron, 1 omitida; cobertura de ramas 80,86 %. CI Windows, source-security y dependency-security aprobaron. Frontend previamente validado 47/47 y smokes/build locales correctos. En CI run 35871086303 los tres escaneos de imagen completaron pero bloquearon por resultados HIGH/CRITICAL (backend 62, frontend 67, backup 162 SARIF); no se aprueba ninguna imagen. El chequeo Compose falló con `KeyError: networks` en su verificador, pendiente corregir. Segundo refuerzo aplica actualización apt en Dockerfiles y añade diagnóstico Compose; 13 tests locales de configuración pasan, queda pendiente nuevo CI Docker. Backup/restore solo local, no off-host. Staging y aprobaciones operativas pendientes.
