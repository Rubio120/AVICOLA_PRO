...
El run `36050121320` para `f89298f` pasó Windows, dependencias, escaneo de código fuente y Compose, pero falló en PostgreSQL por una fixture nueva que insertaba asignaciones antes de persistir sus clasificaciones; ya se añadió un `flush` explícito local, aún pendiente de nuevo CI. Trivy también falló en las tres imágenes, así que no se exportaron imágenes y D12 quedó omitida. Un run anterior (`36045571833`, SHA `18551f7`) sí completó backup/restauración aislada (9 conciliaciones), aunque Trivy también bloqueó entonces las imágenes y el bundle/attestation. D11/D12 siguen abiertas, no hay aprobación técnica para despliegue y el ZIP de prueba final aún no está generado.
...
D11 y D12 siguen ABIERTAS: en el run `36039526802` del SHA `7fa4d34`, el smoke de Compose completo paso; el job de backup fallo al preparar la cuenta sintetica, cuyo CLI tambien omitía el entrypoint de secretos. Esa llamada ya esta corregida y tiene prueba de regresion local. Trivy mantiene HIGH/CRITICAL en las tres imagenes, por lo que backup/restore no terminó y el bundle quedó bloqueado. Esperar un run nuevo. No se desplego el sistema; no esta certificado para produccion.
